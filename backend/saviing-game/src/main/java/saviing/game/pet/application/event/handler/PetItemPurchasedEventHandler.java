...
/**
 * PET 아이템 구매 이벤트 핸들러
 * ItemPurchasedEvent를 listen하여 PET 타입 아이템 구매 시 pet_info 테이블에 데이터를 생성합니다.
 */
@Slf4j
@Component
...
public class PetItemPurchasedEventHandler {
    ...
    /**
     * 아이템 구매 이벤트를 처리합니다.
     * PET 타입 아이템인 경우에만 pet_info 테이블에 펫 정보를 생성합니다.
     *
    ...
    public void handleItemPurchasedEvent(ItemPurchasedEvent event) {
        ...
        try {
            ...
            if (item.getItemType() != ItemType.PET) {
                log.debug("Not a PET item, skipping pet_info creation: itemType={}", item.getItemType());
                return;
            }

            log.info("PET item purchased, creating pet_info: characterId={}, itemId={}, itemName={}",
                event.getCharacterId().value(), event.getItemId().value(), event.getItemName());

            // 3. 실제 인벤토리 아이템 ID 사용
            InventoryItemId inventoryItemId = event.getInventoryItemId();

            // 4. PetCommandService를 통해 펫 정보 생성
            CreatePetCommand command = CreatePetCommand.of(inventoryItemId);
            petCommandService.createPet(command);

            log.info("Successfully created pet_info: characterId={}, itemId={}, inventoryItemId={}",
                event.getCharacterId().value(), event.getItemId().value(), inventoryItemId.value());

        } catch (IllegalStateException e) {
            // 이미 존재하는 펫 정보인 경우 (중복 생성 방지)
            log.warn("PetInfo already exists: characterId={}, itemId={}, error={}",
            ...
            // PET 정보 생성 실패는 전체 구매 프로세스를 실패시키지 않도록 예외를 던지지 않음
            // 대신 로그만 남기고 계속 진행
        }
    }
}