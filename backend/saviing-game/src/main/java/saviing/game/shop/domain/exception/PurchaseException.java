...
public class PurchaseException extends ShopException {
     ...
     * 잔액 부족 예외를 생성합니다.
     */
    public static PurchaseException insufficientFunds(Long characterId, String paymentMethod) {
        String message = String.format("잔액이 부족합니다. 캐릭터: %d, 결제수단: %s", characterId, paymentMethod);
     ...
     */
    public static PurchaseException paymentMethodNotSupported(Long itemId, String paymentMethod) {
        String message = String.format("아이템 %d는 %s로 구매할 수 없습니다", itemId, paymentMethod);
        return new PurchaseException(
            "COIN".equals(paymentMethod) ?
