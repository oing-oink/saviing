 ...
 *
 * 사용자의 게임 데이터와 재화 정보를 전역 상태로 관리하고,
 * 재화 사용 및 업데이트를 위한 액션들을 제공합니다.
 */
interface GameState {
  /** 사용자 게임 데이터 (null이면 아직 로드되지 않음) */
  gameData: UserGameData | null;
  /** 게임 데이터를 스토어에 설정하는 액션 */
  setGameData: (data: UserGameData) => void;
  /** 코인을 사용하는 액션 */
 ...
 *
 * 사용자의 게임 정보와 보유 재화를 관리하고,
 * 재화 사용 시 수량을 안전하게 변경(음수 방지)합니다.
 */
export const useGameStore = create<GameState>(set => ({
  gameData: null,

  setGameData: (data: UserGameData) => set({ gameData: data }),
