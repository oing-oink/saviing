...
export const useGestures = ({
  ...
  const getBoundedPosition = (
    ...
    const maxPanX = Math.max(0, 
      (targetRef.current.offsetWidth * currentScale - containerRef.current.clientWidth) / 2,
    );
    const maxPanY = Math.max(0,
      (targetRef.current.offsetHeight * currentScale - containerRef.current.clientHeight) / 2,
    );
  ...
  useEffect(() => {
    ...
    const onWheel = (e: Event) => handleWheel(e as unknown as React.WheelEvent<HTMLElement>);
    const onMouseDown = (e: Event) => handleMouseDown(e as unknown as React.MouseEvent<HTMLElement>);
    const onMouseMove = (e: Event) => handleMouseMove(e as unknown as React.MouseEvent<HTMLElement>);
    const onMouseUpOrLeave = () => handleMouseUpOrLeave();
    const onTouchStart = (e: Event) => handleTouchStart(e as unknown as React.TouchEvent<HTMLElement>);
    const onTouchMove = (e: Event) => handleTouchMove(e as unknown as React.TouchEvent<HTMLElement>);
