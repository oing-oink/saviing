...
const ItemDetailModal = ({ item, isOpen, onClose }: ItemDetailModalProps) => {
  ...
  return (
    <div className="game fixed inset-0 z-50 flex items-center justify-center bg-white/50">
      <div className="mx-4 max-w-md justify-center rounded-lg bg-secondary p-6 px-1 shadow-lg">
        <div className="mb-4 flex justify-end">
          <button
            onClick={onClose}
            className="text-gray-500 hover:text-gray-700"
          >
            <img src={closeButton} alt="closeButton" className="w-[70%]" />
          </button>
        </div>
        ...
        <div className="flex flex-col items-center pt-2">
          ...
          <div className="mt-5 rounded-3xl bg-primary p-2 px-10">PREVIEW</div>
          <Coin />
        </div>
      </div>
    </div>
